...
func ShowPiInitForm(d PiDetected, packages []string) (config.PiConfig, error) {
	...
	groups := []*huh.Group{huh.NewGroup(
		huh.NewMultiSelect[string]().
			...
			Description("Subagents, MCP adapter, web search, bigpowers skills, permission system, plannotator plan review, live todo overlay. Space toggles, Enter confirms.").
			Options(pkgOptions...).
			Value(&cfg.Packages),
		huh.NewMultiSelect[string]().
