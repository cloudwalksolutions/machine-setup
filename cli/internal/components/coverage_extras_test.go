...
var _ = Describe("AllPullable", func() {
	It("lists the nine components in pull order", func() {
		opts, _, _ := newOpts()
		var names []string
		for _, c := range components.AllPullable(opts) {
			names = append(names, c.Name())
		}
		Expect(names).To(Equal([]string{"vim", "zsh", "byobu", "nvim", "fonts", "terminal", "profiles", "claude", "pi"}))
	})
})
...
var _ = Describe("Zsh branch coverage", func() {
	...
	It("Push skips a missing local profile", func() {
		opts, repoRoot, home := newOpts()
		seedRepo(repoRoot)
		...
		b, err := os.ReadFile(filepath.Join(repoRoot, "zsh", "zshrc"))
		Expect(err).NotTo(HaveOccurred())
		Expect(string(b)).To(Equal("rc"))
	})
